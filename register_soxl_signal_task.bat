@echo off
rem Registers a Windows Task Scheduler task that runs signal_generator.py
rem on weekdays at 07:30 (KST, after US regular market close).
rem soxl_signal_task.xml already defines the triggers (Mon-Fri 07:30), the
rem run-as account (S4U, no password required), and the "run task as soon
...
set "TASK_NAME=SOXLSignalGenerator"
set "SCRIPT_DIR=%~dp0"
set "XML_PATH=%SCRIPT_DIR%soxl_signal_task.xml"
